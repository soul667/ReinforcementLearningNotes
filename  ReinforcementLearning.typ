...
#include "cha1.typ"
// $  P[i,j,k]&=p_pi (s_k|s_i,a_j) \ 
// Pi[k,l]&=pi (a_l|s_k)q_pi (s_k,a_l) \
// $
// So the action value equation can be rewritten as:
// $  q_pi (s_i ,a_j) &= r_pi (s_i,a_j) + gamma sum _k ^("len"(cal(S)))sum _l ^("len"(cal(A))) P[i,k] Q[k,l] Pi[k,l] \ &= r_pi (s_i,a_j)+gamma P[i,:] Q  $
